...
%||S*h-r||_{2}^{2}
S = toeplitz(sTrainSent(1,1:E).', zeros(1,L)); % S 
...
sTrainReceived = sTrainReceived(1:E,1); %lo corto a su long correspondiente
...
r2 = r.';
for k=1:P
    u(:,k) = inv(H_estimada)*double(r2(k,:)).';
end
u(:,P+1) = [ sTrainReceived zeros(1,M-E) ]; %lo que recibi ultima linea
b2 = uint8(u.');
%imshow(b2);
